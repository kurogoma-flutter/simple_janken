...
class JankenHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    bool _isAiko = args?.isAiko ?? false;
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // ヘッダーの文字
          Text(
            _isAiko == false ? '最初はグー！じゃんけん...' : 'あいこで...',
...
class JankenPanel extends StatelessWidget {
  const JankenPanel({Key? key, required this.args, required this.index}) : super(key: key);
...
class JankenResult extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    int _index = args.index; // 選択したカードのインデックス
    int _winCount = args.winCount; // 連勝記録
    int _enemyIndex = _randomIndex(); // ランダム生成した相手のカード
    bool _isAiko = false; // 一旦リセットする
          ...
          children: [
            ...
            GestureDetector(
              ...
              onTap: () => Navigator.pushNamed(
                context,
                "/home",
                arguments: ResultArguments(_winCount, _isAiko),
              ),
            ),
            Text(
              '連勝$_winCount回',
